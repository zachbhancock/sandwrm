...
For individuals dealing with short-read, RADseq-like data, if you filter and assemble your SNPs in a program like `STACKS`, there are options to output VCFs with variant and invariation sites (function `--vcf-all` in the `populations` package). From this, users can use a program such as `pixy` (see the [documentation](https://pixy.readthedocs.io/en/latest/)), which estimates pairwise genetic distance from a VCF. 
